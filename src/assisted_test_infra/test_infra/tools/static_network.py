...
def _prepare_host_static_network_data(
    ...
    *,
    ...
    interfaces = _prepare_interfaces(machine_cidrs, provisioning_cidrs, count, vlan_enabled=vlan_enabled, vlan_id=vlan_id)
...
def _prepare_bonded_interfaces(
    ...
    *,
    ...
    if vlan_enabled:
        vlan_iface = _prepare_vlan_interface(parent_logical_iface="bond0", vlan_id=vlan_id, cidrs=machine_cidrs, count=count)
...
def _prepare_interface(
    ...
    if vlan_enabled and logical_interface == _PRIMARY_LOGICAL_INTERFACE:
        ...
        vlan_iface = _prepare_vlan_interface(parent_logical_iface=logical_interface, vlan_id=vlan_id, cidrs=cidrs, count=host_count)
...
def _prepare_dns_resolver(machine_cidrs: List[str]) -> Dict[str, Dict[str, List[str]]]:
    ...
    servers = [dns_ip]
    # Add host resolv.conf nameservers to mitigate dnsmasq issues on libvirt NAT
    try:
