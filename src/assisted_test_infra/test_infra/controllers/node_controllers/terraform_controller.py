...
class TerraformController(LibvirtController):
    ...
    def _create_nodes(self, running=True):
        ...
        self.tf.apply()
        # If VLAN static IPs flow is enabled, ensure host VLAN sub-interface exists on the libvirt bridge
        if getattr(self._config, "static_ips_vlan", False):
            try:
                self._ensure_vlan_interface()
            except Exception:
                log.exception("Failed creating VLAN sub-interface on host")
